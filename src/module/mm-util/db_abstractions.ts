...
export class ActorInventoryWrapper<T extends LancerItemType> extends EntityCollectionWrapper<T> {
  ...
  async update(items: Array<LiveEntryTypes<T>>): Promise<void> {
    if(this.for_compendium) {
      console.warn("Warning: Cannot currently edit owned items of compendium actors. Re-examine this with Foundry .8, as there was mention of this changing with further tweaks to active effects");
      return;
    }
