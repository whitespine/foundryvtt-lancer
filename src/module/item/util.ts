...
import { LancerSkillData, LancerTalentData, LancerCoreBonusData, LancerPilotArmorData, LancerPilotWeaponData, LancerPilotGearData, LancerFrameData, LancerMechSystemData, LancerMechWeaponData, LancerNPCTemplateData, LancerNPCClassData, LancerNPCFeatureData } from "../interfaces";
...
async function get_pack<T>(pack_name: string): Promise<T[]> {
  return game.packs.get(pack_name).getContent().then((g) => g.map((v) => v.data)) as Promise<T[]>;
}

// Quick accessors to entire arrays of rhe descript items
export async function get_Skills_pack(): Promise<LancerSkill[]> { return get_pack(SKILLS_PACK); }
export async function get_Talents_pack(): Promise<LancerTalent[]> { return get_pack(TALENTS_PACK); }
export async function get_CoreBonuses_pack(): Promise<LancerCoreBonus[]> { return get_pack(CORE_BONUS_PACK); }
export async function get_PilotArmor_pack(): Promise<LancerPilotArmor[]> { return get_pack(PILOT_ARMOR_PACK); }
export async function get_PilotWeapons_pack(): Promise<LancerPilotWeapon[]> { return get_pack(PILOT_WEAPON_PACK); }
export async function get_PilotGear_pack(): Promise<LancerPilotGear[]> { return get_pack(PILOT_GEAR_PACK); }
export async function get_Frames_pack(): Promise<LancerFrame[]> { return get_pack(FRAME_PACK); }
export async function get_MechSystems_pack(): Promise<LancerMechSystem[]> { return get_pack(MECH_SYSTEM_PACK); }
export async function get_MechWeapons_pack(): Promise<LancerMechWeapon[]> { return get_pack(MECH_WEAPON_PACK); }
export async function get_NpcClassses_pack(): Promise<LancerNPCClass[]> { return get_pack(NPC_CLASS_PACK); }
export async function get_NpcTemplates_pack(): Promise<LancerNPCTemplate[]> { return get_pack(NPC_TEMPLATE_PACK); }
export async function get_NpcFeatures_pack(): Promise<LancerNPCFeature[]> { return get_pack(NPC_FEATURE_PACK); }
...
export function compendium_item_to_lancer_item_data(x: Skill | Talent | CoreBonus | License | PilotArmor | PilotWeapon | PilotGear | Frame | MechWeapon | MechSystem | NpcClass | NpcTemplate | NpcFeature): LancerItemData {
...
export async function CompendiumData_as_ContentPack(): Promise<IContentPack> {
  ...
  let coreBonuses = raw_core_bonuses.map(c => conv.LancerCoreBonusData_to_ICoreBonusData(c.data.data));
  ...
  let npcClasses = raw_npc_classes.map(c => conv.LancerNPCClassData_to_INpcClassData(c.data.data));
  let npcFeatures = raw_npc_features.map(c => conv.LancerNPCFeatureData_to_INpcFeatureData(c.data.data));
  let npcTemplates = raw_npc_templates.map(c => conv.LancerNPCTemplateData_to_INpcTemplateData(c.data.data));
  ...
  let pilotWeapons = raw_pilot_weapons.map(c => conv.LancerPilotWeaponData_to_IPilotEquipment(c.data.data));
