...
export function single_action_editor(path: string, helper: HelperData) {
  ...
  const qs = (key: string) =>
    std_string_input(`${path}.${key}`, ext_helper_hash(helper, { label: key }));
  const qn = (key: string) =>
    std_num_input(`${path}.${key}`, ext_helper_hash(helper, { label: key }));
  const qc = (key: string) =>
    std_checkbox(`${path}.${key}`, ext_helper_hash(helper, { label: key }));
  ...
  let inputs: string[] = [
    ...
    std_enum_select(
      `${path}.Activation`,
      ActivationType,
      ext_helper_hash(helper, { default: ActivationType.None })
    ),
    qn("Cost"),
 ...
 */
export function single_action_preview(
  item_path: string,
  action_path: string,
  helper: HelperData
): string {
  ...
  if (helper.hash["macro-actor"]) {
    ...
    let macro_ctx: ActionMacroCtx = {
      ...
      name,
      icon: icon_spec
        ? `/systems/lancer/assets/icons/${icon_spec.replace(/-/g, "_")}.svg`
        : undefined,
    };
    let action_class = icon_spec ? `cci cci-${icon_spec}` : "fas fa-dice-d20";
    macro = `<a class="lancer-macro i--m ${action_class}" title="${
      action.Activation
    }" ${macro_elt_params(macro_ctx)}> </a>`;
  }
 ...
 */
export function action_list_display(
  item_path: string,
  actions_path: string,
  helper: HelperData
): string {
    ...
    return `
        <div class="card action-list">
            <div class="lancer-header">
                ...
                ${inc_if(
                  `<a class="gen-control fas fa-plus" data-action="append" data-path="${item_path}.${actions_path}" data-action-value="(struct)action"></a>`,
                  edit
                )}
            </div>
            ${body}
...
export function HANDLER_activate_edit_action<T>(
  ...
  actions.on("click", async event => {
    ...
    return GenericEditDialogue.render_form(
      sheet_data,
      single_action_editor(action_path, {
        data: {
          root: sheet_data,
        },
        hash: {},
