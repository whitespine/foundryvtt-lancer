...
import { categorize } from "../item/util";
...
export class LancerNPCSheet extends ActorSheet {
    ...
    activateListeners(html: any) {
        ...
        if (this.actor.owner) {
            ...
            tier_selector.change((ev: any) => {
                ...
                NPCClassStats = (actor.items.find((i: Item) => i.type === "npc_class") as any).data
                    .data.stats;
    ...
    async _onDrop(event: any) {
        ...
        if (item) {
            ...
            if (item.type === "npc_class") {
                ...
                if (data.pack) {
                    const npcClass = (await actor.importItemFromCollection(
                        data.pack,
                        data.id
                    )) as any;
    ...
    _updateObject(event: Event | JQuery.Event, formData: any): Promise<any> {
        ...
        else if (this.actor.img == token.img && this.actor.img != formData.img) {
            this.actor.update({ "token.img": formData.img });
        }
